...
def get_wikitext2(tokenizer, nsamples, seed, seqlen, path=None):
    ...
    dataset = load_dataset('parquet', data_files={
        'train': f'/work/home/xdb4_97730/ljc/data/wikitext2/test-00000-of-00001.parquet',
        'test': f'/work/home/xdb4_97730/ljc/data/wikitext2/train-00000-of-00001.parquet'
    })
...
def get_pileval(tokenizer, nsamples, seed, path, seqlen=512):
    ...
    for data in dataset:
        ...
        for conversation in conversations:
            # 提取 "value" 字段
            line = conversation.get('value', '').strip()
            if not line:
                continue
...
def get_pileval(tokenizer, nsamples, seed, path, seqlen=512):
    ...
    # 将所有样本合并，并确保它们在同一设备上
    cat_samples = torch.cat(samples, dim=1).to(device)
    n_split = cat_samples.shape[1] // seqlen
    print(f' * Split into {n_split} blocks')

    # 返回分割后的样本，保持在同一设备
    return [
        cat_samples[:, i * seqlen:(i + 1) * seqlen].to(device) for i in range(n_split)
    ], None
...
def get_calib_loaders(name,
    ...
    if 'pileval' in name:
        if path is None:
            ...
            path = "/work/home/xdb4_97730/ljc/models/data/ShareGPT_V3_unfiltered_cleaned_split.json"
